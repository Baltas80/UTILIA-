...
void main() {
  group('UTILIA calculations', () {
    ...
    test('tip with zero people is safe', () => expect(tipPerPerson(100, 10, 0), 0));

    test('loan payment zero interest', () => expect(loanPayment(1200, 0, 12), 100));
    test('loan payment standard', () => expect(loanPayment(10000, 12, 12), closeTo(888.4879, .001)));
    test('loan with invalid months is safe', () => expect(loanPayment(1200, 5, 0), 0));

    test('compound interest', () => expect(compound(1000, 10, 2), closeTo(1210, 0.001)));
    test('compound zero years returns principal', () => expect(compound(1000, 10, 0), 1000));
    ...
    test('electricity fractional power', () => expect(electricityCost(500, 4, 30, .25), 15));
    ...
    test('cost per km zero distance is safe', () => expect(costPerKm(45, 0), 0));
    ...
    test('rule of three zero divisor is safe', () => expect(ruleOfThree(0, 3, 4), 0));
    ...
    test('date difference', () {
      ...
      expect(dateDifferenceDays(DateTime(2026, 1, 11), DateTime(2026, 1, 1)), 10);
      expect(dateDifferenceDays(DateTime(2024, 2, 28), DateTime(2024, 3, 1)), 2);
    });

    test('work hours', () => expect(workHours(8, 17, 60), 8));
    test('work hours crossing midnight', () => expect(workHours(22, 6, 0), 8));
    test('negative resulting work hours are safe', () => expect(workHours(8, 8, 60), 0));
    ...
    test('length imperial conversion', () => expect(convertLength(1, 'mi', 'km'), closeTo(1.609344, .000001)));
    ...
    test('weight imperial conversion', () => expect(convertWeight(1, 'lb', 'kg'), closeTo(.45359237, .00000001)));
