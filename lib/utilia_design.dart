...
class UtiliaBrand {
  ...
  static const categoryColors = <String, Color>{
    'Dinero': Color(0xFF18B77B), 'Tiempo': Color(0xFF7654E8), 'Casa': Color(0xFFFF8A17),
    'Coche': Color(0xFF1686EA), 'Conversores': Color(0xFF12AFAF), 'Salud': Color(0xFFE9426F),
    'Estudio': Color(0xFFE8A900), 'Varios': Color(0xFF68778C),
  };
...
ThemeData utiliaTheme(Brightness brightness) {
  ...
  final scheme = ColorScheme.fromSeed(seedColor: UtiliaBrand.blue, brightness: brightness).copyWith(
    primary: UtiliaBrand.blue, onPrimary: Colors.white, surface: dark ? const Color(0xFF0C1726) : Colors.white,
    surfaceContainer: dark ? const Color(0xFF101E30) : const Color(0xFFF1F5FB),
    surfaceContainerHighest: dark ? const Color(0xFF1B2A3B) : const Color(0xFFEAF0F8), surfaceTint: Colors.transparent,
  );
  ...
  return ThemeData(
    useMaterial3: true, colorScheme: scheme, scaffoldBackgroundColor: dark ? UtiliaBrand.darkBackground : UtiliaBrand.lightBackground,
...
class _MountainPainter extends CustomPainter {
  @override void paint(Canvas canvas, Size size) {
    ...
    final peak = Offset(size.width * .42, size.height * .20); final snow = Path()..moveTo(peak.dx, peak.dy)..lineTo(peak.dx - size.width * .07, peak.dy + size.height * .13)..lineTo(peak.dx - size.width * .015, peak.dy + size.height * .09)..lineTo(peak.dx + size.width * .045, peak.dy + size.height * .15)..close(); canvas.drawPath(snow, Paint()..color = Colors.white.withValues(alpha: .78));
  }
