...
class _CalculatorPageState extends State<CalculatorPage> {
  ...
  bool get _isConverter =>
      widget.tool.type == ToolType.length ||
  ...
  bool _validateNumericInputs() {
    ...
    switch (widget.tool.type) {
      ...
      case ToolType.countdown:
        return _validInteger(controllers[0].text) &&
            _validInteger(controllers[1].text) &&
            _validInteger(controllers[2].text);
      default:
        return true;
    }
  }
  ...
  Future<void> calculate() async {
    ...
    switch (widget.tool.type) {
      case ToolType.percentage:
        ...
        break;
      case ToolType.discount:
        r = discountedPrice(x, y);
        break;
      case ToolType.iva:
        r = priceWithIva(x, y);
        break;
      case ToolType.tip:
        r = tipPerPerson(x, y, z.toInt());
        break;
      case ToolType.loan:
        r = loanPayment(x, y, z.toInt());
        break;
      case ToolType.compoundInterest:
        r = compound(x, y, z.toInt());
        break;
      case ToolType.area:
        r = area(x, y);
        u = 'm²';
      ...
      case ToolType.age:
        ...
        if (d == null) {
          _error(widget.s.invalidDate);
          return;
        }
        if (d.isAfter(DateTime.now())) {
          _error(_futureDateMessage);
          return;
        }
        r = ageInYears(d);
        u = _unit('años', 'years', 'ans', 'Jahre', 'anni', 'anos');
        break;
      case ToolType.dateDifference:
        ...
        r = dateDifferenceDays(a, b).toDouble();
        u = _unit('días', 'days', 'jours', 'Tage', 'giorni', 'dias');
        break;
      case ToolType.workHours:
        r = workHours(x, y, z);
        u = 'h';
        break;
      case ToolType.countdown:
        r = countdownSeconds(x.toInt(), y.toInt(), z.toInt()).toDouble();
        u = 's';
        break;
      case ToolType.length:
