...
class _UtiliaHomePageState extends State<UtiliaHomePage> {
  ...
  Widget _brand() => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('UTILIA', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontSize: 21, fontWeight: FontWeight.w900, height: 1)),
        const SizedBox(height: 3),
        Text(_subtitle, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 9.5, fontWeight: FontWeight.w700)),
      ])),
      IconButton(visualDensity: VisualDensity.compact, onPressed: () => setState(() => tab = 3), icon: const Icon(Icons.settings_outlined)),
    ]),
  );
  ...
  Widget _home() {
    ...
    return SafeArea(child: ListView(padding: const EdgeInsets.only(bottom: 20), children: [
      _brand(),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 18), child: _hero()),
      const SizedBox(height: 12),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 18), child: _benefits()),
      const SizedBox(height: 19),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 18), child: UtiliaSectionTitle(title: text('Acceso rápido', 'Quick access', 'Accès rapide', 'Schnellzugriff', 'Accesso rapido', 'Acesso rápido'), action: TextButton(style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 4)), onPressed: () => setState(() => tab = 1), child: Text(_seeAll)))),
      const SizedBox(height: 5),
      SizedBox(height: 108, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 18), scrollDirection: Axis.horizontal, itemCount: quick.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, i) => _quick(quick[i]))),
      const SizedBox(height: 18),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 18), child: UtiliaSectionTitle(title: s.categories, action: TextButton(style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 4)), onPressed: () {}, child: Text(_seeAll)))),
      const SizedBox(height: 7),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 18), child: GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: categories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: 1.12), itemBuilder: (_, i) => _category(categories[i]))),
    ]));
  }

  Widget _hero() => Container(height: 222, clipBehavior: Clip.antiAlias, decoration: BoxDecoration(borderRadius: BorderRadius.circular(22), boxShadow: [BoxShadow(color: UtiliaBrand.blue.withValues(alpha: .15), blurRadius: 22, offset: const Offset(0, 10))]), child: Stack(children: [
    const Positioned.fill(child: UtiliaMountainArtwork(borderRadius: 0)),
    Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.transparent, Color(0xE0000000)])))),
    Positioned(left: 18, right: 18, bottom: 17, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(_eyebrow, style: TextStyle(color: Colors.white.withValues(alpha: .82), fontSize: 8.5, fontWeight: FontWeight.w900, letterSpacing: .95)),
      const SizedBox(height: 6),
      Text(s.slogan, style: const TextStyle(color: Colors.white, fontSize: 24, height: 1.03, fontWeight: FontWeight.w900)),
      const SizedBox(height: 7),
      Text(_heroSub, style: const TextStyle(color: Colors.white, fontSize: 12.5, fontWeight: FontWeight.w600)),
    ])),
  ]));

  Widget _benefits() => Row(children: [
    _benefit(Icons.bolt_rounded, text('Útil', 'Useful', 'Utile', 'Nützlich', 'Utile', 'Útil')),
    const SizedBox(width: 8),
    _benefit(Icons.favorite_border_rounded, text('Simple', 'Simple', 'Simple', 'Einfach', 'Semplice', 'Simples')),
    const SizedBox(width: 8),
    _benefit(Icons.all_inclusive_rounded, text('Siempre contigo', 'Always with you', 'Toujours avec vous', 'Immer bei dir', 'Sempre con te', 'Sempre consigo')),
  ]);
  Widget _benefit(IconData icon, String label) => Expanded(child: Container(height: 55, decoration: BoxDecoration(color: Theme.of(context).colorScheme.surfaceContainer, borderRadius: BorderRadius.circular(15)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 19, color: UtiliaBrand.blue), const SizedBox(width: 5), Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.labelLarge?.copyWith(fontSize: 9.5, fontWeight: FontWeight.w700)))])));

  Widget _quick(UtiliaTool tool) {
    final color = tool.type == ToolType.scientificCalculator || tool.type == ToolType.ruleOfThree ? UtiliaBrand.violet : UtiliaBrand.blue;
    return SizedBox(width: 96, child: Card(margin: EdgeInsets.zero, child: InkWell(borderRadius: BorderRadius.circular(16), onTap: () => openUtiliaTool(context, tool, widget.storage, _load, s), child: Padding(padding: const EdgeInsets.fromLTRB(7, 10, 7, 8), child: Column(children: [UtiliaSoftIcon(icon: tool.icon, color: color, size: 46), const Spacer(), Text(name(tool), textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.labelLarge?.copyWith(fontSize: 9.5, height: 1.1))])))));
  }

  Widget _category(String category) { final list = tools.where((t) => t.category == category).toList(); final color = UtiliaBrand.categoryColor(category, list.first.tint); return Card(margin: EdgeInsets.zero, color: color.withValues(alpha: .10), clipBehavior: Clip.antiAlias, child: InkWell(borderRadius: BorderRadius.circular(14), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => UtiliaCategoryPage(category: category, tools: list, favorites: favorites, onFavorite: _favorite, storage: widget.storage, onHistory: _load, s: s))), child: Padding(padding: const EdgeInsets.all(13), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [UtiliaSoftIcon(icon: list.first.icon, color: color, size: 48), const Spacer(), Text(s.category(category), maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontSize: 14, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Row(children: [Expanded(child: Text(s.toolsCount(list.length), style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 9.5))), Icon(Icons.chevron_right_rounded, color: color, size: 18)])])))); }
  ...
  Widget _toolTile(UtiliaTool tool) { final color = UtiliaBrand.categoryColor(tool.category, tool.tint); final fav = favorites.contains(tool.type); return Padding(padding: const EdgeInsets.only(bottom: 8), child: Card(child: InkWell(borderRadius: BorderRadius.circular(17), onTap: () => openUtiliaTool(context, tool, widget.storage, _load, s), child: Padding(padding: const EdgeInsets.all(10), child: Row(children: [UtiliaSoftIcon(icon: tool.icon, color: color, size: 44), const SizedBox(width: 11), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name(tool), style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: 2), Text(desc(tool), maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium)])), IconButton(onPressed: () => _favorite(tool.type), icon: Icon(fav ? Icons.favorite_rounded : Icons.favorite_border_rounded, color: fav ? const Color(0xFFE93D68) : Theme.of(context).colorScheme.outline))]))))); }
  ...
  Widget _more() => SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 28), children: [
    Text(s.more, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
    const SizedBox(height: 14),
    Card(child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [const UtiliaLogoMark(size: 52), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('UTILIA', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text('v0.5.3', style: Theme.of(context).textTheme.bodyMedium)])), IconButton(onPressed: () => widget.onTheme(!widget.darkMode), icon: Icon(widget.darkMode ? Icons.dark_mode_rounded : Icons.light_mode_outlined, color: UtiliaBrand.blue))]))),
    const SizedBox(height: 10),
    _setting(Icons.settings_outlined, text('Ajustes', 'Settings', 'Réglages', 'Einstellungen', 'Impostazioni', 'Definições'), text('Tema, idioma y preferencias', 'Theme, language and preferences', 'Thème, langue et préférences', 'Thema, Sprache und Einstellungen', 'Tema, lingua e preferenze', 'Tema, idioma e preferências'), _pickLanguage),
    const SizedBox(height: 8),
    _setting(Icons.new_releases_outlined, text('Novedades', 'What’s new', 'Nouveautés', 'Neuigkeiten', 'Novità', 'Novidades'), text('Ver qué hay de nuevo', 'See what’s new', 'Voir les nouveautés', 'Neuigkeiten ansehen', 'Scopri le novità', 'Ver novidades'), null),
    const SizedBox(height: 8),
    _setting(Icons.star_outline_rounded, text('Valora UTILIA', 'Rate UTILIA', 'Évaluer UTILIA', 'UTILIA bewerten', 'Valuta UTILIA', 'Avaliar UTILIA'), text('Tu opinión nos ayuda', 'Your opinion helps us', 'Votre avis nous aide', 'Ihre Meinung hilft uns', 'La tua opinione ci aiuta', 'A sua opinião ajuda-nos'), null),
    const SizedBox(height: 8),
    _setting(Icons.share_outlined, text('Compartir app', 'Share app', 'Partager l’app', 'App teilen', 'Condividi app', 'Partilhar app'), text('Recomienda UTILIA', 'Recommend UTILIA', 'Recommandez UTILIA', 'UTILIA empfehlen', 'Consiglia UTILIA', 'Recomende UTILIA'), null),
    const SizedBox(height: 8),
    _setting(Icons.privacy_tip_outlined, text('Política de privacidad', 'Privacy policy', 'Politique de confidentialité', 'Datenschutzerklärung', 'Privacy', 'Política de privacidade'), text('Tus datos, tu control', 'Your data, your control', 'Vos données, votre contrôle', 'Ihre Daten, Ihre Kontrolle', 'I tuoi dati, il tuo controllo', 'Os seus dados, o seu controlo'), null),
    const SizedBox(height: 8),
    _setting(Icons.info_outline_rounded, text('Acerca de', 'About', 'À propos', 'Über', 'Informazioni', 'Sobre'), 'UTILIA v0.5.3', null),
  ]));
  Widget _setting(IconData icon, String title, String subtitle, VoidCallback? onTap) => Card(child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 2), leading: UtiliaSoftIcon(icon: icon, color: UtiliaBrand.blue, size: 43), title: Text(title, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)), subtitle: Text(subtitle, style: Theme.of(context).textTheme.bodyMedium), trailing: const Icon(Icons.chevron_right_rounded), onTap: onTap));
