...
class _UtiliaHomePageState extends State<UtiliaHomePage> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() { favorites = f.map(_type).whereType<ToolType>().toSet(); history = h; });
  }
  ...
  Widget _dashboard() {
    ...
    final quick = quickTypes.map((type) => tools.where((t) => t.type == type).firstOrNull).whereType<UtiliaTool>().toList();
    return ListView(
      ...
      children: [
        ...
        GridView.builder(
          ...
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.18),
  ...
  Widget _hero() => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: UtiliaBrand.blue.withValues(alpha: .18), blurRadius: 28, offset: const Offset(0, 14))],
        ),
        child: Stack(
          children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
              Text('HERRAMIENTAS PARA TU DÍA A DÍA', style: TextStyle(color: Colors.white.withValues(alpha: .76), fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
              const SizedBox(height: 10),
              Text(s.slogan, style: const TextStyle(color: Colors.white, fontSize: 25, height: 1.05, fontWeight: FontWeight.w900, letterSpacing: -.7)),
              const SizedBox(height: 11),
              Text('Todo lo útil, en un solo lugar.', style: TextStyle(color: Colors.white.withValues(alpha: .90), fontSize: 14, fontWeight: FontWeight.w600)),
            ]),
          ],
        ),
      );
      ...
      child: Card(
        child: InkWell(
          ...
          child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 38, height: 38, decoration: BoxDecoration(gradient: LinearGradient(colors: [accent.withValues(alpha: .18), accent.withValues(alpha: .07)]), borderRadius: BorderRadius.circular(13)), child: Icon(tool.icon, color: accent, size: 22)),
            const Spacer(),
            Text(name(tool), maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.labelLarge),
          ])),
        ),
      ),
    );
  ...
  Widget _categoryCard(String category) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => UtiliaCategoryPage(category: category, tools: list, favorites: favorites, onFavorite: _favorite, storage: widget.storage, onHistory: _load, s: s))),
        child: Padding(
          padding: const EdgeInsets.all(15),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 48, height: 48, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [accent.withValues(alpha: .20), accent.withValues(alpha: .06)]), borderRadius: BorderRadius.circular(16)), child: Icon(list.first.icon, color: accent, size: 26)),
            const Spacer(),
            Text(cat(category), maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 3),
            Row(children: [Expanded(child: Text(s.toolsCount(list.length), style: Theme.of(context).textTheme.bodyMedium)), Icon(Icons.arrow_forward_rounded, size: 18, color: accent)]),
          ]),
        ),
      ),
    );
      ...
      child: Card(
        child: InkWell(
          ...
          child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
            Container(width: 50, height: 50, decoration: BoxDecoration(color: accent.withValues(alpha: .11), borderRadius: BorderRadius.circular(16)), child: Icon(tool.icon, color: accent, size: 25)),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name(tool), style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: 3), Text(desc(tool), maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium)])),
            IconButton(onPressed: () => _favorite(tool.type), icon: Icon(favorite ? Icons.favorite_rounded : Icons.favorite_border_rounded, color: favorite ? accent : Theme.of(context).colorScheme.outline)),
          ])),
        ),
      ),
    );
  ...
  Widget _favoritesView() => favorites.isEmpty
      ? _empty(Icons.favorite_border_rounded, s.emptyFavorites)
      : ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 28), children: [Text(s.favorites, style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 16), ...tools.where((t) => favorites.contains(t.type)).map(_toolTile)]);
  ...
  Widget _historyView() => history.isEmpty
      ...
      : ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 28), children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(s.history, style: Theme.of(context).textTheme.headlineSmall), TextButton.icon(onPressed: () async { await widget.storage.clearHistory(); await _load(); }, icon: const Icon(Icons.delete_outline_rounded), label: Text(s.clearHistory))]),
          const SizedBox(height: 12),
  ...
  Widget _moreView() => ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 28), children: [
  ...
  Future<void> _pickLanguage() async {
    final value = await showDialog<UtiliaLanguage>(context: context, builder: (c) => SimpleDialog(title: Text(s.languageLabel), children: UtiliaLanguage.values.map((l) => SimpleDialogOption(onPressed: () => Navigator.pop(c, l), child: Text(UtiliaStrings(l).languageName))).toList()));
