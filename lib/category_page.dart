...
class _UtiliaCategoryPageState extends State<UtiliaCategoryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: SafeArea(child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(12, 4, 12, 4), child: Row(children: [IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.arrow_back_rounded)), Expanded(child: Text(category, style: Theme.of(context).textTheme.titleLarge)), IconButton(onPressed: () {}, icon: const Icon(Icons.search_rounded))])),
        Padding(padding: const EdgeInsets.fromLTRB(16, 4, 16, 12), child: Align(alignment: Alignment.centerLeft, child: Text(text('Todo en su lugar, para tu día a día.', 'Everything in its place, for everyday life.', 'Tout à sa place, pour votre quotidien.', 'Alles an seinem Platz für deinen Alltag.', 'Tutto al suo posto, per la vita di ogni giorno.', 'Tudo no seu lugar, para o seu dia a dia.'), style: Theme.of(context).textTheme.headlineSmall))),
  ...
  Widget _toolCard(BuildContext context, UtiliaTool tool) {
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 8), child: Card(child: InkWell(borderRadius: BorderRadius.circular(20), onTap: () => openUtiliaTool(context, tool, widget.storage, widget.onHistory, widget.s), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 9), child: Row(children: [
