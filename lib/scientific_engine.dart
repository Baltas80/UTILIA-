...
class _Parser {
  ...
  double _primary() {
    ...
    final start = pos;
    while (pos < source.length && RegExp(r'[0-9.,]').hasMatch(source[pos]))
      pos++;
    if (start != pos) {
    ...
    final nameStart = pos;
    while (pos < source.length && RegExp(r'[A-Za-z]').hasMatch(source[pos]))
      pos++;
    ...
    switch (name) {
      case 'sin':
        return math.sin(_angle(x));
      case 'cos':
        return math.cos(_angle(x));
      case 'tan':
        return math.tan(_angle(x));
      case 'asin':
        return _outAngle(math.asin(x));
      case 'acos':
        return _outAngle(math.acos(x));
      case 'atan':
        return _outAngle(math.atan(x));
      case 'sqrt':
        return math.sqrt(x);
      case 'ln':
        return math.log(x);
      ...
      default:
        throw FormatException('Función desconocida: $name');
    }
  }
  ...
  void _skip() {
    while (pos < source.length && source[pos].trim().isEmpty) pos++;
  }
